    ...
    app.post(
        ...
        async (request, reply) => {
            try {
                ...
                const similarityQuery = sql<number>`1 - (${schema.audioChunks.embeddings} <=> ${embeddingsAsString}::vector)`
                ...
                if (!insertedQuestion) {
                    return reply
                        .status(500)
                        .send({ error: 'Failed to create question' })
                }
                ...
                return reply
                    .status(500)
                    .send({ error: `Failed to process question${error}` })
            }
        }
    )
}
